...
// TODO: Import real API service functions once backend endpoints are ready
// import { fetchAdminDonations, deleteDonation } from '../services/api';
...
const AdminDonationPage = () => {
  ...
  // Get authenticated admin info from localStorage
  const user = JSON.parse(localStorage.getItem('user') || '{}');
  const adminName = user.username || 'Admin Name';

  // Temporary mock data for UI/UX testing and design preview
  const [donations, setDonations] = useState([
    { id: 1, prod: 'Prod_1', user: 'User_1', date: '21/04/2026', qty: '1Kg', status: 'Active', coords: '35.9311, 0.0892', category: 'Fruit', color: 'green' },
    { id: 2, prod: 'Prod_2', user: 'User_2', date: '21/04/2026', qty: '2Pieces', status: 'Expired', coords: '35.9345, 0.0912', category: 'Pastries', color: 'red' },
    { id: 3, prod: 'Prod_3', user: 'User_3', date: '21/04/2026', qty: '2Kg', status: 'Expired', coords: '35.9280, 0.0850', category: 'Milk', color: 'orange' },
    { id: 4, prod: 'Prod_4', user: 'User_4', date: '21/04/2026', qty: '2Kg', status: 'Donated', coords: '35.9390, 0.0960', category: 'Fruit', color: 'green' }
  ]);

  // Hook to fetch live data from the backend (Uncomment when API is integrated)
  /*
  useEffect(() => {
    const getDonations = async () => {
      try {
        const data = await fetchAdminDonations();
        setDonations(data); // Replaces mock data with real database records
      } catch (err) {
  ...
  */
  ...
  // Handle donation post deletion (Backend-ready)
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this donation?")) {
      try {
        // await deleteDonation(id); // API call to remove from database
        setDonations(donations.filter(item => item.id !== id)); // Update UI state
  ...
  // Get matching badge color depending on post status
  const getStatusColor = (status) => {
    if (status === 'Active') return '#000000'; 
    if (status === 'Expired') return '#F4A261'; 
    if (status === 'Donated') return '#588157'; 
    return '#000000';
  };
  ...
  const filteredDonations = donations.filter(item => {
    ...
    const matchesSearch = item.prod.toLowerCase().includes(searchLower) || 
                ...
                {filteredDonations.length > 0 ? (
                  filteredDonations.map(item => (
                    <tr key={item.id}>
                      <td>
                        <img 
                          src="https://images.unsplash.com/photo-1542838132-92c53300491e?w=100" 
                          alt={item.prod} 
                      ...
                      <td style={{ fontWeight: '600', color: getStatusColor(item.status) }}>
                        {item.status}
                      </td>
